...
END_DESC
      ...
      task :generate_bookings => :environment do
        return puts "project should be selected" unless ENV['project']
        ...
        amount.times do
          ...
          total_allocated = ResourceBooking.where(assigned_to_id: user_id).where('start_date < :ed and end_date >= :sd', sd: start_date, ed: end_date + 1).sum(:booking_value)
