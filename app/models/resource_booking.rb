...
class ResourceBooking < ApplicationRecord
  ...
  acts_as_activity_provider(
    ...
    scope: joins(:project, :assigned_to, :author).
      includes(:issue)
  )

  safe_attributes 'project_id', 'assigned_to_id', 'issue_id', 'start_date', 'end_date', 'booking_value', 'notes', 'booking_type'

  validates :project_id, :assigned_to_id, :start_date, :booking_value, presence: true
  validates :booking_type, inclusion: { in: %w[hours percent], message: :invalid_booking_type }

  validates :booking_value, numericality: {
    allow_nil: true,
    greater_than_or_equal_to: 0,
    less_than_or_equal_to: ->(booking) { booking.max_booking_value }
  }

  validate :check_interval
  validate :check_issue_params # Adds warnings instead of errors

  before_validation :convert_total_hours_to_hours
  ...
  def working_days_amount
    @working_days_amount ||= if RedmineResources.people_pro_plugin_installed?
      person = assigned_to.becomes(Person)
      Dayoff.person_work_hours_per_day(person, start_date, get_end_date).values.count{|h| h > 0}
    else
      RedmineResources.global_work_days(start_date, get_end_date).count
    end
  end
  ...
  def total_hours
    working_days_amount * daily_hours
  end
  ...
  def max_booking_value
    booking_type == 'hours' ? 24 : 100
  end

  def daily_hours(workday_length = 8)
  booking_value
  end
  ...
  def check_issue_params
    if start_date && !expired? && issue.try(:due_date) && issue.due_date < get_end_date.to_date
  ...
  def convert_total_hours_to_hours
    if booking_type == 'total_hours'
      if working_days_amount > 0
        self.booking_value = (booking_value.to_f / working_days_amount).round(2)
        self.booking_type = 'hours'
      else
        errors.add(:base, :invalid_working_days, message: 'The booking period must include at least one working day.')
      end
    end
  end
end
